...
; Here we write the roll number
	...
	mov eax,ecx			; Put it in eax so we can make a string.
	call intToString		; Now eax is the intBuffer pointer.
...
; Here we read in a value
	call readRand			; Call the read random function

; Mod that value down to what you would expect for it to be
	mov eax,[current]		; Put our number into eax
	mov ecx,[nSides]		; Put edx as the number of sides
	mov edx,0			; Set edx to zero to prevent div issue
	div ecx				; Divide that eax value
	mov eax,edx			; The remainder is our roll	

; Add that value to the total
	add [total],eax			; Add this number to the total.	
...
; Print that value
	...
	mov ebx,[randHandle]		; Prepare to close the file
; Close the file
...
; Reads in a value from the random file into the current variable.
; Requires that the file be open.
; Uses all four main registers and does not return anything.
readRand:
	mov eax,3			; Prepare to read
	mov ebx,[randHandle]		; read in from /dev/urandom
